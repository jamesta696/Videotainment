class VideoItem {
    constructor(data) {
        ...
        this.videoThumb.addEventListener(
            "click",
            e => this.onVideoRedirect(e),
            false
        );
        this.videoTitle.addEventListener(
            "click",
            e => this.onVideoRedirect(e),
            false
        );
        this.element.addEventListener(
            "click",
            e => this.onVideoItemChannelRedirect(e),
            false
        );
        ...
        <div class="row row-search">
                <div class="search-column-left">
                    <div class="placeholder-16-9-320">
                        <div class="img-in-placeholder"><a href="/player.html?id=${
                            data.id.videoId
                        }"><img src="${
            data.snippet.thumbnails.high.url
        }"></a></div>
                        <div class="duration">${data.snippet.duration ||
                            ""}</div>
                    </div>
                </div>
                <div class="search-column-right">
                    <h3 class="search-title padding-top-0 line-clamp-2-search"><a href="">${
                        data.snippet.title
                    }</a></h3>
                    <p class="search-subtitle"><a href="channel.html?id=${
                        data.snippet.channelId
                    }">${data.snippet.channelTitle}</a></p>
                    <p class="search-views"></p>
                    <p class="search-desc line-clamp-3">${
    ...
    setViewCount(data) {
        let viewCountContainer = this.element.querySelector(
            ".VideoItem .search-views"
        );
        viewCountContainer =
            data.statistics.viewCount == undefined
                ? (viewCountContainer.innerHTML = "")
                : (viewCountContainer.innerHTML =
    ...
    setDate(data) {
        let dateContainer = this.element.querySelector(
            ".VideoItem .search-views"
        );
