class PlayerApplication extends Application {
    ...
    onLoad() {
        ...
        this.relatedVideosContainer = this.element.querySelector(
            ".column-video-right"
        );
    ...
    onFormatLinks(res) {
        ...
        if (this.videoDescription.innerHTML.includes("http")) {
            this.videoDescription.innerHTML = res.items[0].snippet.description.replace(
                replacePattern1,
    ...
    onFormatVideoTags(res) {
        ...
        for (let i in res.items[0].snippet.tags) {
            tagLinks =
                tagLinks +
                `<a href="/results.html?keyword=${
                    res.items[0].snippet.tags[i]
                }">${res.items[0].snippet.tags[i]}</a>, `;
        }

        this.videoTags =
            res.items[0].snippet.tags == undefined
                ? (this.videoTags.innerHTML = "")
